...
# Derived from the same files the seed script globs, so adding a card can't
# be forgotten here (adding Discover's six required remembering to extend a
# 115-entry hand list — issue #154). The guard below keeps a broken glob from
# silently turning this suite into "asserts the API returns nothing".
CARD_IDS = [
    json.loads(p.read_text())["id"]
    for p in sorted(
        (Path(__file__).parent.parent.parent / "backend" / "data" / "cards").glob("**/*.json")
    )
    if "staging" not in p.parts
]
assert len(CARD_IDS) >= 100, "card catalog glob found almost nothing — path broken?"
